...
def ml_clustering(spark_df, spark):
    dir_path = "../analysis/ml_clustering"
    ...
    customer_group_data = customer_type.fit(spark_df).transform(spark_df)
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(f"{dir_path}/customer_depending_city_amount_type_features.png", dpi=300, bbox_inches="tight")
    plt.show()
    plt.close()
    ...
    ax.legend()
    plt.savefig(f"{dir_path}/customer_similar_buy.png", dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
    ...
    for pc in ["PC1", "PC2", "PC3"]:
        ...
        plt.tight_layout()
        output_path = f"{dir_path}/top_bottom_{pc}_contributions_decoded_original.png"
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(f"{dir_path}/time_clusters.png", dpi=300, bbox_inches="tight")
    plt.show()
